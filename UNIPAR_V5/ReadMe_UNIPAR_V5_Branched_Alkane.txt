...
Main module will also call for an input file for a specific chamber simulation data found in the input folder (i.e. MMDDYY_East or MMDDYY_West)
	Which contains concentrations of HO2 (PPB), RO2 (PPB), change in ROG (dROG, PPB), Temp (K), RH (0-1), dSO4 (umol m^(-3)), dNH4 (umol/m^(-3))
	Each column has a value for each 6 minute cycle. The preset value is 100 cycles. 
