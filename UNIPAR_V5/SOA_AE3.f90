!:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
! SOA_AE3.f90 copyrightable work product created by faculty at the University of Florida. 
!
...
!  REVISION HISTORY:
! FSB This module contains all of the information and subroutines 
!     used in subroutine NEWT. 
!           
! YOJ 07/31/02 changed FUNCTION FMIN to SUBROUTINE FMINV to avoid errors
!     with (some) compilers
!
! CGN 01/12/04 removed ITS from NEWT call vector, added documentation, 
!     and removed extraneous lines of code -- Dr. Chris G. Nolte
!
!  REFERENCES:
!   1. Schell, B., I. J. Ackermann, H. Hass, F. S. Binkowski, and
...
!      Res., Vol 106, No D22, 28275-28293, 2001.
      ...
      INTEGER MAXITS            !bs maximum number of iterations
       PARAMETER (MAXITS = 1000)  !500 ! 100 original   CF

      REAL TOLF                 !bs convergence criterion on function values
       PARAMETER (TOLF = 1.E-13) !-13  !1.E-9  orginal CF

      REAL TOLMIN                  !bs criterion whether spurious convergence to
       PARAMETER (TOLMIN = 1.E-16) !bs a minimum of fminv has occurred    !1.E-12 original  CF

      REAL TOLX                 !bs convergence criterion on delta_x
       PARAMETER (TOLX = 1.E-15)   !1.E-10   original  CF

      REAL STPMX                !bs scaled maximum step length allowed
       PARAMETER (STPMX = 100.)
      
      INTEGER NP
       PARAMETER( NP = 51 ) ! this variable must equal NCVAP !!
...
!bs ** BS ** ** BS ** BS ** ** BS ** BS ** ** BS ** BS ** ** BS ** BS *!
      SUBROUTINE NEWT(LAYER, X, N, CHECK, CTOT, CSAT, IMWCV, MINITW)
!bs ** BS ** ** BS ** BS ** ** BS ** BS ** ** BS ** BS ** ** BS ** BS *!
!bs                                                                    !
!bs  Description:                                                      !
!bs                                                                    !
...
!  REVISION HISTORY:
!
! CGN 01/12/04  Removed ITS from call vector, changed B1 & B2 to 
!      scalars, added documentation, removed extraneous lines of code
...
!bs
      ...
      REAL IMW
      REAL M
      COMMON /NEWTINP/ CT(NP), CS(NP), IMW(NP), M
      SAVE /NEWTINP/
!bs
...
!bs
      ...
      DO ITS = 1, MAXITS        !start of iteration loop
         ...
         CALL LNSRCH(CTOT, N, XOLD, FOLD, G, &
                     !LNSRCH returns new X, F.It also 
                     P, X, F, STPMAX, &
                     !calculates FVEC at new X when it
                     CHECK)           !calls FMINV
         TEST = 0.
         DO I = 1, N
            IF (ABS(FVEC(I)) .GT. TEST) TEST = ABS(FVEC(I))
...
!bs ** BS ** ** BS ** BS ** ** BS ** BS ** ** BS ** BS ** ** BS ** BS *!
!
!   From Equation (8) of Schell et al., 2001:
!     Caer,i = Ctot,i - Csat,i * (Caer,i/MWi) / 
!                             ( sum_j (Caer,j/MWj) + Cinit/MWinit)
!   Let Xi  = Caer,i
!       a_i = 1 / MWi
!       M   = Cinit/MWinit
!       CTi = Ctot,i
!       CSi = Csat,i
!   Then,       
!       Xi  = CTi - CSi * (a_i * Xi) / ( sum_j (a_j * Xj) + M )
!
!   Multiply above equation by sum_j(a_j*Xj) + M and group terms
!       a_i Xi^2 + ( sum_jnei (a_j*Xj) + M + CSi*a_i - CTi*a_i ) Xi
!                - CTi * ( sum_jnei (a_j*Xj) + M ) = 0
!
!   This equation is of the form F(X) = a_i*Xi^2 + b_i*Xi + c_i = 0.
!     F(X) is stored as FVEC in this subroutine.
!
!   See also FDJAC.

!  REVISION HISTORY:
!
! CGN 01/12/04  Added documentation, removed extraneous lines of code
...
!bs ** BS ** ** BS ** BS ** ** BS ** BS ** ** BS ** BS ** ** BS ** BS *!
!bs
!    call vector modified to remove NP and set dimensions  to N.   
      IMPLICIT NONE
!bs
      INTEGER N, INDX(N)
...
!bs
      IMPLICIT NONE
!bs
!c   call vector modified to remove NP. All dimensions now depend upon N only.
      INTEGER N, INDX(N)
!c      INTEGER NMAX
